...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def safe_answer(*args, **kwargs):
        ...
        answered = True
        try:
            return await orig_answer(*args, **kwargs)
        except Exception:
            pass

    query.answer = safe_answer
    try:
        await _handle_callback_impl(update, context)
    finally:
        if not answered:
            try:
                await orig_answer()
...
async def _handle_callback_impl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    data = query.data
