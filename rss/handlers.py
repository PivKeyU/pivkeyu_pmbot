...
def _who(user_id) -> str:
    ...
    RSS 命令管理员与授权用户都能用，所以读者不一定是管理员。

    注意：订阅表以**字符串** chat_id 为键（``get_chat_id`` 返回 ``str(chat.id)``），
    而 ``config.ADMIN_IDS`` 存的是 int。直接 ``user_id in config.ADMIN_IDS`` 会恒为
    False，管理员也会被叫成「客人」，所以这里必须转换后比较。
    """
    try:
        return copy_text.address(int(user_id) in config.ADMIN_IDS)
    except (TypeError, ValueError):
        return copy_text.address(False)
...
async def _ensure_access(update: Update):
    ...
    if not is_authorized(user_id):
        await message.reply_text(
            copy_text.with_deco("客人还没有 RSS 茶点间的通行证哦。", 'DENY'))
        return None

    if not rss_settings.is_enabled():
        await message.reply_text(
            copy_text.with_deco(
                "RSS 女仆正在休息，请联系管理员女仆长在 /panel → RSS 订阅茶点管理 中唤醒。", 'WAIT'))
        return None
...
async def add_feed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await message.reply_text(copy_text.with_deco(
            f"{_who(chat_id)}，请给女仆 RSS 链接。女仆小抄：/rss_add <链接>", 'ASK'))
        return

    feed_url = context.args[0]
    if not is_valid_url(feed_url):
        await message.reply_text(copy_text.with_deco(
            f"这条链接 '{feed_url}' 看起来不太对，{_who(chat_id)}检查一下再交给女仆吧。", 'ERROR'))
        return
    ...
    if feed_url in subscriptions_data[chat_id]["rss_feeds"]:
        await message.reply_text(copy_text.with_deco(
            f"这份茶点 {feed_url} 已经在{_who(chat_id)}的订阅单里啦。", 'OK'))
        return
    ...
    reply_message_text = copy_text.with_deco(
        f"茶点 '{feed_title}' ({feed_url}) 已端上订阅单啦。", 'OK')
...
async def list_feeds(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not feeds:
        await message.reply_text(copy_text.with_deco(
            f"{_who(chat_id)}还没有任何 RSS 茶点。使用 /rss_add <链接> 添加一份吧。", 'EMPTY'))
        return

    message_content = f"{_who(chat_id)}当前的 RSS 茶点单:\n"
    ...
    await message.reply_text(copy_text.with_deco_head(message_content, 'GREET'))
...
async def remove_feed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await message.reply_text(copy_text.with_deco(
            f"{_who(chat_id)}，请告诉女仆要撤下的 RSS 链接或 ID（来自 /rss_list）。女仆小抄：/rss_remove <链接或ID>", 'ASK'))
        return
    ...
    if not feeds:
        await message.reply_text(copy_text.with_deco(
            f"{_who(chat_id)}还没有可以撤下的 RSS 茶点。", 'EMPTY'))
        return
    ...
    if feed_to_remove:
        ...
        data_manager.save_subscriptions(_get_data_file(context))
        reply_message_text = copy_text.with_deco(
            f"茶点 '{removed_title}' 已经撤下啦。", 'OK')
        logger.info("用户 %s 移除了订阅源: %s", chat_id, feed_to_remove)
    else:
        reply_message_text = copy_text.with_deco(
            f"女仆没找到标识符为 '{identifier}' 的茶点。请用 /rss_list 查看茶点 ID/链接。", 'ERROR')
...
async def add_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) < 2:
        await message.reply_text(copy_text.with_deco(
            "女仆小抄：/rss_addkeyword <RSS链接或ID> <口味词>", 'ASK'))
        return
    ...
    if not feeds:
        await message.reply_text(copy_text.with_deco(
            f"{_who(chat_id)}还没有能添加口味词的 RSS 茶点。", 'EMPTY'))
        return

    target_feed_url = find_feed_by_identifier(feed_identifier, feeds)

    if not target_feed_url:
        await message.reply_text(copy_text.rss_feed_not_found(feed_identifier))
        return
    ...
    if keyword_to_add in feed_data["keywords"]:
        ...
        await message.reply_text(copy_text.with_deco(
            f"口味词 '{keyword_to_add}' 已经在 '{feed_title}' 里啦。", 'OK'))
    else:
        feed_data["keywords"].append(keyword_to_add)
        data_manager.save_subscriptions(_get_data_file(context))
        feed_title = feed_data.get("title", target_feed_url)
        await message.reply_text(copy_text.with_deco(
            f"口味词 '{keyword_to_add}' 已加入 '{feed_title}'。", 'OK'))
...
async def remove_keyword(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) < 2:
        await message.reply_text(copy_text.with_deco(
            "女仆小抄：/rss_removekeyword <RSS链接或ID> <口味词>", 'ASK'))
        return
    ...
    if not feeds:
        await message.reply_text(copy_text.with_deco(
            f"{_who(chat_id)}还没有能删除口味词的 RSS 茶点。", 'EMPTY'))
        return
    ...
    if keyword_to_remove in feed_data.get("keywords", []):
        ...
        data_manager.save_subscriptions(_get_data_file(context))
        await message.reply_text(copy_text.with_deco(
            f"口味词 '{keyword_to_remove}' 已从 '{feed_title}' 删除。", 'OK'))
        logger.info("用户 %s 从订阅源 %s 移除了关键词 '%s'", chat_id, target_feed_url, keyword_to_remove)
    else:
        await message.reply_text(copy_text.with_deco(
            f"女仆没在 '{feed_title}' 里找到口味词 '{keyword_to_remove}'。", 'ERROR'))
...
async def list_keywords(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await message.reply_text(copy_text.with_deco(
            "女仆小抄：/rss_listkeywords <RSS链接或ID>", 'ASK'))
        return
    ...
    if not feeds:
        await message.reply_text(copy_text.rss_no_feeds(_who(chat_id)))
        return
    ...
    await message.reply_text(copy_text.with_deco_head(reply_message_text, 'OK'))
...
async def remove_all_keywords(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await message.reply_text(copy_text.with_deco(
            "女仆小抄：/rss_removeallkeywords <RSS链接或ID>", 'ASK'))
        return
    ...
    if feed_data.get("keywords"):
        ...
        data_manager.save_subscriptions(_get_data_file(context))
        await message.reply_text(copy_text.with_deco(
            f"茶点 '{feed_title}' 的所有口味词都已清空。", 'OK'))
        logger.info("用户 %s 移除了订阅源 %s 的所有关键词。", chat_id, target_feed_url)
    else:
        await message.reply_text(copy_text.with_deco(
            f"茶点 '{feed_title}' 原本就没有口味词。", 'EMPTY'))
...
async def set_custom_footer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if footer_text:
        reply_message_text = copy_text.with_deco_head(f"RSS 小尾巴已系好：\n{footer_text}", 'OK')
    else:
        reply_message_text = copy_text.with_deco("RSS 小尾巴已解下。", 'OK')
...
async def toggle_link_preview(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_message_text = copy_text.with_deco(f"链接预览已切换为：{status_text}。", 'OK')
...
async def add_authorized_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or user.id not in config.ADMIN_IDS:
        await message.reply_text(
            copy_text.with_deco("只有管理员女仆长可以登记 RSS 授权客人。", 'DENY'))
        return

    if not context.args:
        await message.reply_text(copy_text.with_deco(
            "女仆小抄：/rss_add_user <user_id>", 'ASK'))
        return

    try:
        target_id = int(context.args[0])
    except ValueError:
        await message.reply_text(copy_text.rss_user_id_invalid())
        return

    added = rss_settings.add_authorized_user(target_id)
    if added:
        await message.reply_text(copy_text.auth_granted(target_id, copy_text.AUTH_LIST_RSS))
        logger.info("管理员 %s 添加了 RSS 授权用户 %s", user.id, target_id)
    else:
        await message.reply_text(copy_text.auth_already(target_id, copy_text.AUTH_LIST_RSS))
...
async def remove_authorized_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or user.id not in config.ADMIN_IDS:
        await message.reply_text(
            copy_text.with_deco("只有管理员女仆长可以移除 RSS 授权客人。", 'DENY'))
        return

    if not context.args:
        await message.reply_text(copy_text.with_deco(
            "女仆小抄：/rss_rm_user <user_id>", 'ASK'))
        return
    ...
    if removed:
        await message.reply_text(copy_text.auth_revoked(target_id, copy_text.AUTH_LIST_RSS))
        logger.info("管理员 %s 移除了 RSS 授权用户 %s", user.id, target_id)
    else:
        await message.reply_text(copy_text.auth_missing(target_id, copy_text.AUTH_LIST_RSS))
